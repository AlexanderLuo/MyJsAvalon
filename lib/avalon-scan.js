 ...
 **********************************************************************/
    ...
    de("扫描启动：制定开始节点与对应VM",elem.outerHTML,vmodel);
    elem = elem || root
...
function createSignalTower(elem, vmodel) {
    //todo 这里创造了avalonctrl 标识,然而去掉了也没有影响功能的实现
    console.log("这个所谓的createSignalTower不知道啥意思，avalonctrl就是他设置的")
...
function executeBindings(bindings, vmodels) {
    ...
    for (var i = 0, data; data = bindings[i++]; ) {
        ...
        console.log("宏执行：单条宏进行预处理Handler")
...
function scanAttr(elem, vmodels) {
    ...
    console.log('二级扫描(属性):捕获节点所有属性',attributes)
    ...
    for (var i = 0, attr; attr = attributes[i++]; ) {
        ...
        if (attr.specified) {
            ...
            if (match = attr.name.match(rmsAttr)) {
                console.log("二级扫描(属性):avalon标签过滤,为"+match[0]+"构建宏指令")

                //如果是以指定前缀命名的
                var type = match[1]    //正则设置了捕获组  这里是 ms-``type``--param
                console.log(match[0]+"标签type类型为：",type)
                var param = match[2] || "" //正则设置了捕获组  这里是 ms-type--```param```
                console.log(match[0]+"标签param值为：",param)
                ...
                //todo 似乎是版本迭代产物，未遇到
                if (events[type]) {
                    console.log('·····该avalon标签'+type+'属于事件·····')
                    console.log("未遇到111")
                    param = type
                    type = "on"
                } else if (obsoleteAttrs[type]) {
                    log("ms-" + type + "已经被废弃,请使用ms-attr-*代替")
                    console.log("未遇到1113252")
                ...
                if (typeof bindingHandlers[type] === "function") {
                    ...
                    var binding = {
                        type: type,     //类型     attr
                        param: param,   //参数     value
                        element: elem,  //dom
                        name: match[0], //full ms-attr-value
                        value: value,   //key  绑定到的字段名字  name/schoolCode之类的
                        priority: type in priorityMap ? priorityMap[type] : type.charCodeAt(0) * 10 + (Number(param) || 0)   //优先级 是一个数字
                    }
                    console.log("标签合法验证通过，进行宏构建:",binding);



                    if (type === "html" || type === "text") {
                        console.log('处理ms-html  ms-text')
                    ...
                    if (name === "ms-if-loop") {
                        console.log("处理ms-if-loop")
                        binding.priority += 100
                    }


                    if (vmodels.length) {
                        console.log('宏压入bindings数组')
                        bindings.push(binding)
                        if (type === "widget") {
                            console.log("处理ms-widget")
    ...
    console.log('bindings数组根据宏的优先级排序')
    //宏指令排序， 根据定义的数字优先级
    ...
    for (i = 0; binding = bindings[i]; i++) {
        console.log("宏分发：根据type分发第"+i+"个宏")
        type = binding.type

        if (rnoscanAttrBinding.test(type)) {
            console.log("type分发A线条 /^if|widget|repeat$/ ")
            return executeBindings(bindings.slice(0, i + 1), vmodels)
        }
        else if (scanNode) {
            console.log("type分发B线条,非/^if|widget|repeat$/ ")
            console.log("若type /^each|with|html|include$/，则后续不走B线条了 ")
            //scanNode= true//false
...
function scanTag(elem, vmodels, node) {
    ...
    //无法调用getAttributeNode方法将结束，即无法使用
    //兼容

    // ingore 那个Object自定义上传就属于老式的
    ...
    //例子ms-controller="test"
    //getAttribute("ms-controller") 获得节点的值,类型为string,结果:test
    //getAttributes(elem)获取节点所有属性,对象(类似于fileList这种有索引)，avalon自定义方法
    //getAttributeNode("ms-controller") 获得整个属性节点，类型为object 结果：ms-controller="test"  由name-value组成的2元对象



    //进行一级扫描
    var a = elem.getAttribute("ms-skip")
    de('一级扫描(控制器):'+'对'+elem.tagName+'节点进行  ms-skip捕捉 其值：'+a)
    var b = elem.getAttributeNode("ms-important")
    de('一级扫描(控制器):'+'从'+elem.tagName+'节点进行 ms-important捕捉 其值：'+b)
    var c = elem.getAttributeNode("ms-controller")
    de('一级扫描(控制器):从'+elem.tagName+'标签 捕捉ms-controller元素节点：',c)
    ...
    if (typeof a === "string") {
        ...
        de('一级扫描结束,移除ms-controller ,ms-import标签')
...
avalon.fn.mix({
    ...
    toggleClass: function(value, stateVal) {
        //ms-class的底层实现，对DOM进行添加，或者移除class
...
function parseExpr(code, scopes, data) {
    ...
    if (!assigns.length && dataType === "duplex") {
        ...
        console.log("未知mark11")

        return
    }
    if (dataType !== "duplex" && (code.indexOf("||") > -1 || code.indexOf("&&") > -1)) {

        console.log("未知mark2222")
...
function parseExprProxy(code, scopes, data, tokens, noregister) {
    ...
    console.log("宏执行:parse代理分发")

    if (Array.isArray(tokens)) {
        console.log("智能代理未知功能A")
    ...
    if (data.evaluator && !noregister) {
        ...
        console.log("智能代理未知功能B",data.handlerName)
...
bindingHandlers.attr = function (data, vmodels) {
    ...
    console.log("1.获取宏value(指向VM的字段):"+data.value.trim())
    var text = data.value.trim(),
            simple = true

    if (text.indexOf(openTag) > -1 && text.indexOf(closeTag) > 2) {
        console.log("宏执行的未知功能A")
        simple = false
...
bindingHandlers["class"] = function(data, vmodels) {
    ...
    if (!oldStyle || isFinite(oldStyle)) {
        ...
        if (colonIndex === -1) { // 比如 ms-class="aaa bbb ccc" 的情况
            console.log("ms-class处理。没有:条件式情况")
            var className = text
        }
        else {
            console.log("ms-class处理。带有:条件式情况")
            // 比如 ms-class-1="ui-state-active:checked" 的情况
...
//亦即，先进入loading阶段的script标签(模块)必然会先进入loaded阶段
        ...
        if (url) {
            ...
            factory.require(url)
4        } else {//合并前后的safari，合并后的IE6-9走此分支
