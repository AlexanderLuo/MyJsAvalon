...
//avalon scan  处理的是avalon标签相关，采用递归节点的方式进行处理


avalon.scan = function(elem, vmodel, group) {

    //  这个  root可是恐怖的html整个文档树，object对象！！！！！！
    //console.log(root)
    //他妈的 你这一步到底干了什么 为什么 就成了 dody树了 草
...
function createSignalTower(elem, vmodel) {
    console.log("这个所谓的createSignalTower不知道啥意思")
...
function scanAttr(elem, vmodels) {
    ...
    console.log('对所有的attributes标签扫描,不管是不是avalon的',attributes)
    ...
    for (var i = 0, attr; attr = attributes[i++]; ) {
        ...
        if (attr.specified) {
            ...
            if (match = attr.name.match(rmsAttr)) {
                ...
                console.log("avalon节点type:",type)
                var param = match[2] || "" //正则设置了捕获组  这里是 ms-type--```param```
                console.log("avalon节点param:",param)
                ...
                if (events[type]) {
                    console.log('·····该avalon节点'+type+'属于事件·····')
                    param = type
                    type = "on"
                ...
                if (typeof bindingHandlers[type] === "function") {
                    console.log('进入了这里 也不明白什么叫bindingHandlers[type]是个函数？')
                    ...
                    if (type === "html" || type === "text") {
                        console.log('type=html 也就是  ms-html  ms-text 的逻辑在这里！！！')
                    ...
                    if (name === "ms-if-loop") {
                        console.log("ms-if-loop的逻辑在这里···········")
                        binding.priority += 100
                    }


                    if (vmodels.length) {
                        console.log('在VM存在下创建宏数组')
    ...
    for (i = 0; binding = bindings[i]; i++) {
       type = binding.type
        console.log(type+"如果你看到的是on ,那么处理的是事件逻辑")
...
function scanNodeList(parent, vmodels) {
    ...
    //上一步传入的玩意儿  elem未指定就是root咯。就是狗日的整颗树咯
    ...
    while (node) {
        var nextNode = node.nextSibling        //妈卖批的被选节点的下一个同级节点（在相同树层级中的下一个节点），大概是兄弟的感脚
...
function scanNode(node, nodeType, vmodels) {
    //递归了 递归了 递归了 操他妈的 递归了 递归了
    console.log("递归了 递归scamTag 了了了了了了")
    if (nodeType === 1) {
...
function scanTag(elem, vmodels, node) {
    ...
    //getAttributeNode 该方法只能从当前节点上取属性 不能从子节点取， root是html节点，懂了吗？
    //getAttribute 一样的
    ...
    var a = elem.getAttribute("ms-skip")
            ...
            console.log('从'+elem.tagName+'捕捉ms-skip元素节点：'+a)
    ...
    var b = elem.getAttributeNode("ms-important")
            ...
            console.log('从'+elem.tagName+'捕捉ms-important元素节点：'+b)

    var c = elem.getAttributeNode("ms-controller")

            // ms-controller="jo"  这就是一个属性节点,object对象
            console.log('从'+elem.tagName+'捕捉ms-controller元素节点：',c)
    ...
    if (typeof a === "string") {
        ...
        */
        ...
        console.log('从扫描到的节点取得对应的vmodel',newVmodel)
        if (!newVmodel) {
            console.log("傻逼 你没这个VM 滚")
            return
        }
        ...
        console.log('本轮扫描结束,移除ms-controller ,ms-import标签~~')
