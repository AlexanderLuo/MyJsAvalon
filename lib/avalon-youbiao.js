...
//=============================css相关=======================
    ...
    function uniqSet(array) {
        ...
        for (var i = 0; i < array.length; i++) {
            ...
            if(navigator.userAgent.indexOf("MSIE 8.0")>0) {     //#Alexander fix no-text    ie 下ms-wedget兼容
                if(typeof id === "string" ){
                    if (!unique[id]) {
                    ...
                    var op=id.id+"test"
                    if (!unique[op]) {
                        unique[op] = ret.push(el)
                    }
                }
            }else{
                //ie 10+ 对象自转换为字符串，且引用指向各个对象，判重
                if (!unique[id]) {
                    unique[id] = ret.push(el)
                }
...
//处理radio, checkbox, text, textarea, password
    duplexBinding.INPUT = function(element, evaluator, data) {
        ...
        function compositionEnd() {
            ...
            //    if(val){
            //        element.value=val+" ";//先改变值
            //        element.value=val;//再恢复原值,如果没有改变值,直接这么写,光标还是不会发生变化
            //    }
            //}

            setTimeout(updateVModel);               //fix #Alexrander  Luo #1770   中文绑定解决

        }

        function changeFloat(va){
            evaluator(va)



            setTimeout(function(){
                console.log("attach - fixed -no -test #Alexrander Ie10 光标游动")
        ...
        var updateVModel = function() {
            ...
            if ($elem.data("duplexObserve") !== false) {
                changeFloat(lastValue) //附加一层光标移动
                //evaluator(lastValue)
