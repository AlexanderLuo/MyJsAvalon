...
function modelFactory(source, $special, $model) {
    ...
    console.log("1.定义的对象进厂，称作source")
    for(ob in source ){
        console.log('进厂的对象字段及其值：'+ob+':'+source[ob])
    }

    //for(ob in source ){
    //    console.log("list ob "+ob)
    //}

    //sorce里面的就是$id name这些字段的对象
    ...
    console.log('所谓的给非监控数组添加一个$special强制要监听的属性,初始化的是空,其值:'+$skipArray.$special)
    ...
    $$skipArray.forEach(function (name) {
        console.log("挂载前准备，从source中 删除静态的非监控属：他们存在$$skipArray静态声明中，有：  "  +name)
        delete source[name]
    })
    ...
    console.log('2.进入source 加工流程');
    names.forEach(function (name, accessor) {
        console.log('··············对source中每个字段 挂载accessor··············');
        console.log('当前挂载字段:'+name);
        var val = source[name]
        console.log(name+'的值是'+val);

        //把定义的对象字段转移到model中去
        console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        console.log('！！！$model 现身， 存放了source的每个字段的值！！！！')
        console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        $model[name] = val
        for(ob in $model){
            console.log('当前$model中: '+ob+"值为"+$model[ob]);
        }


        if (isObservable(name, val, $skipArray)) {
            console.log('进入inObservable的判定');
            //总共产生三种accessor  存取器
            console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
            console.log('！！！$events 现身， ！！！！')
            console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
            $events[name] = []
            for(ob in $events){
                console.log('初始化事件挂载器,给每一个字段挂在一个$events[name]对象,他们有'+name);
            }
            ...
            if (valueType === "object" && isFunction(val.get) && Object.keys(val).length <= 2) {
                ...
                console.log('进入类型为'+valueType+'的挂载分支');
                accessor = makeSimpleAccessor(name, val)
            }

            //某个字段的存储器
            accessors[name] = accessor
            console.log('当前soucre字段加工完毕，把产品accessor存入accessors[name]  其本质是一个 accessor 访问器函数！！')

        }
    })

    console.log('··············当前字段 挂载accessor结束··············');




    console.log('3.基于accessors ,cource ,$vmodel产生新的$vmodel,可以看出$vmodel与$model的区别')
    /* jshint ignore:end */
    ...
    console.log('$vmodel创建过程 1.  把对应字段的accessor存入   此时  source 的字段值也被替换成了accessor访问器')

    for(ob in $vmodel){
        console.log('字段'+ob+'值:'+ $vmodel[ob] )
    }
    ...
    console.log('对vmodel添加一个$reinitialize函数')
...
function makeSimpleAccessor(name, value) {
    console.log('它被称作简单存储器,我们可以看出 一个访问器其实是一个函数')

    function accessor(value) {

        var oldValue = accessor._value;
        console.log("我就是传说中的存储器，每次数据变化调度到我，上一次值:"+oldValue);
        if (arguments.length > 0) {
            console.log('视图更新分支arguments.length > 0 ');
            ...
            console.log('发现accessor执行完更新后 return了本身this,估计这就是长时间存活在内存中')
            return this
        } else {
            console.log('在arguments长度<0时的分支,无更新分支');
...
function globalUpdateValue(vmodel, value) {
    console.log('这是全局更新函数````````````````````````````````````,操作vmodel，被accessor引用时候指向他本身')


    //像是把accessor里面的value塞到vmodel里去了
    //可以看到vmodel是高层抽象 里面应该还有除了数据之外的 东西，而$model里面却只有数据！

    // 马丹 似乎是黑魔法   被引用的时候 this 指向 被引用的对象， 就是 引用他的那个accessor的对应的 name字段，从而访问到vmodel里面的这个
    //这是调用了set
...
function globalNotify(vmodel, value, oldValue) {
    ...
    console.log('这是全局提示函数````````````````````````````````````，操作vmodel 被accessor引用时候指向他本身')
    var name = this._name
...
function accessorFactory(accessor, name) {
    console.log('这里是accessorFactory 根据字段制造对应的访问器' +
        '一个accessor 访问器 拥有 _name字段_value字段 以及对gloablUpdateValue 与 globalNotify的引用')
    accessor._name = name
    //同时更新_value与model
...
var dependencyDetection = (function () {
        console.log("为啥一开始这里有个自执行函数:dependencyDetection")
    var outerFrames = []
    var currentFrame
    return {
        ...
        collectDependency: function (vmodel, accessor) {
            console.log('进入了一个检验vmodel与当前字段accessor依赖关系的函数')
            if (currentFrame) {
