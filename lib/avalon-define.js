...
var VMODELS = avalon.vmodels = {} //所有vmodel都储存在这里
    ...
    //一个vmodel 是对象实现监控属性后的对象,里面存在普通字段，以及一个称作$reinitialize的回调


avalon.define = function (id, factory) {

    //2种定义方式 avalon.define({obj})  avalon.define(id,function(vm){ vm.各种字段}) 这里id在无funciton冒充了obj
    var $id = id.$id || id
    ...
    else
    {
        ...
        factory(scope) //回调，scope 替代了vm. 各种字段挂在在了scope上

        model = modelFactory(scope) //偷天换日，将scope换为model   转变为了对象定义法
    ...
    return VMODELS[$id] = model //挂载存储空间，为一个model对象,拥有一个$id字段
...
function modelFactory(source, $special, $model) {
    ...
    //初始化时  对象定义 仅1个source 源，即定义的对象


    //应该是一个数组情况下的复用  未发现调用情况  todo
    if (Array.isArray(source)) {
        console.log("modelFactory source isArray")
    ...
    console.log("1.modelFactory source is object  初始化对象")


    //```````````````````````````````````````````````````modelFactory source is object I E6-8 兼容的处理 todo  `````````````````````````````````````````````````````````
    ...
    //获取$skipArry数组
    console.log("2.modelFactory source is object  对象处理: 取出$skipArray ")
    var $skipArray = Array.isArray(source.$skipArray) ? source.$skipArray : []
    console.log('获取$skipArry数组:'+$skipArray)




    // 给非监控数组添加一个$special强制要监听的属性 todo 暂时不知道干嘛的
    $skipArray.$special = $special || {} //强制要监听的属性
    console.log('3.modelFactory source is object 给非监控数组添加一个$special 强制要监听的属性 暂时不知道干嘛的')


    console.log('接下来创建了 $vmodel $model accessors computed')
    ...
    $$skipArray.forEach(function (name) {
        console.log('4.modelFactory source is object  VM准备工作 ，从source中 删除 非监控属  delete source[name]:$id 等 ')
        delete source[name]
    })

    console.log('那么对source的初始化完成了，source变成了:')
    for(ob in source){
        console.log(ob+'其值'+source[ob])
    }
    console.log('source VM准备工作完成························································')
    ...
    console.log('正式进入VM化核心');

    names.forEach(function (name, accessor) {

        console.log('VM化字段:'+name);

        var val = source[name]


        //把定义的对象字段转移到model中去
        console.log('把该字段放入对应的 vmmodels.vmmode.$model,纯净字段储存')

        $model[name] = val

        console.log('5.modelFactory source is object 对象字段可监控的情况下VM化');
        ...
        if (isObservable(name, val, $skipArray)) {
            ...
            console.log('对当前字段:'+name+'其值:'+val+'进行类别判定(avalon.type):'+valueType);
            ...
            if (valueType === "object" && isFunction(val.get) && Object.keys(val).length <= 2) {
                ...
                console.log('进入类型为'+valueType+'的VM化分支，根据key val 产生一个访问器 accessor');
                accessor = makeSimpleAccessor(name, val)
            }


            //某个字段的存储器
            console.log("字段:"+name+"访问器创建完毕，并储存到accessors容器中")
    ...
    console.log("6.modelFactory source is object  访问器创建完毕，Object.defineProperties挂载")


    //核心API,descriptorFactory把字段与访问器结合，即Object.defineProperties,getter/setter指向同一个accessor.通过是否传参分流

    $vmodel = defineProperties($vmodel, descriptorFactory(accessors), source) //生成一个空的ViewModel


    console.log("7.modelFactory source is object  没有访问器的东西，比如函数，直接挂在在VM对象上")
    ...
    console.log("8.modelFactory source is object hideProperty 一系列key")
    //记得在segementdefault 上看到过，是个框架的作者总想把自己的东西隐藏起来，有时真是无聊
    //添加$id, $model, $events, $watch, $unwatch, $fire
    //这个hide把enumerable:是否能在for...in循环中遍历出来或在Object.keys中列举出来 置为了false
    ...
    console.log('9.modelFactory source is object 对vmodel添加一个$reinitialize函数')


    $vmodel.$reinitialize = function () {
        console.log('reinitialize被调用  暂时不知道什么鬼')
...
function makeSimpleAccessor(name, value) {
    console.log('简单访问器的创建分支, 访问器其实就是 retrun 了一个accessor函数')
    console.log("在accessor函数上挂载一个字段，_value，用于储存对象的值");



    //访问器 vm.key 执行arguments为空的访问
    //   vm.key=123 执行arguments不为空的访问
    // 即使传说中的 get/set
    function accessor(value) {
        ...
        if (arguments.length > 0) {
            console.log('访问setter,set的值为：'+value);
            if (!stopRepeatAssign && !isEqual(value, oldValue)) {

                ///发现  这里的this,其实指向的是vmodel  字段+$reinittializ
                console.log("调用setter的 updateValue函数,指向globalUpdataValue")
                accessor.updateValue(this, value)
                console.log("调用setter的 notufy函数,指向globalNotify")
                accessor.notify(this, value, oldValue)


            }
            //todo setter 返回本身
            console.log("setter 调用后 会返回本身这个VM！")
            console.log(this)
            return this
        }
        //getter
...
function globalUpdateValue(vmodel, value) {
    console.log('globalUpdataValue 函数 把vmdodel.$model  与VM accesosor_value 字段值更新为设定值,即纯净对象与accessor都进行更改')
...
function globalNotify(vmodel, value, oldValue) {
    ...
    console.log('globalNotify函数  进行同步视图操作')
    //this为当前访问器
    var name = this._name
...
function accessorFactory(accessor, name) {
    console.log("基础 accessor再高级加工，函数添加._name指向key,添加updateValue函数,notufy函数")
    accessor._name = name
    //同时更新_value与model
...
var isEqual = Object.is || function (v1, v2) {
    ...
    //可VM化的标准
    //1.函数不可VM化，nodeType不可VM化
    //2.$skipArray中存在 不可VM化
    //3. todo $secial这个东西可否VM化
...
var dependencyDetection = (function () {
        console.log("加载依赖调度系统:dependencyDetection,是一个对象")
    var outerFrames = []
    var currentFrame
    return {
        begin: function (accessorObject) {
            console.log("·······begin 依赖")
        ...
        collectDependency: function (vmodel, accessor) {
            console.log('getter 的依赖调度系统，在currentFrame存在下会执行一个回调，没遇到现在')
            if (currentFrame) {
                console.log('执行一个currentFrame回调');
    ...
    if (valueFn) { //如果是求值函数
        dependencyDetection.begin({
            callback: function (vmodel, dependency) {
                console.log('接下来进入了这里，似乎是要注入什么依赖的样子')
                console.log('这里传入了2个对象，第一个长成vmodel.$events[dependency._name]，试试其值:'+vmodel.$events[dependency._name])
                console.log('另外一个似乎有点大 循环一下省略了');
...
function injectDependency(list, data) {
    ...
    if (list && avalon.Array.ensure(list, data) && data.element) {
        console.log('进入这里有些条件判断需要研究,其中提到了data.element，看看他们是啥,太大了 省略')
        //for(ob in data.element){
        //    console.log(ob+':'+data.element[ob])
        //}

        injectDisposeQueue(data, list)
        if (new Date() - beginTime > 444 ) {
            console.log('这又是什么鬼，什么叫new Date-beginTime>444分支？时差？')
            rejectDisposeQueue()
        }
    }
}
...
function fireDependencies(list) {
    ...
    console.log('这是更新视图的函数吧？出现了个所谓的订阅者 好像很复杂的样子')



    if (list && list.length) {
        if (new Date() - beginTime > 444 && typeof list[0] === "object") {
            console.log("在444ms上 执行  rejectDisponseQueue函数")
            rejectDisposeQueue()
        }
...
function rejectDisposeQueue(data) {
    ...
    while (data = disposeQueue[--i]) {
        console.log('让我看看disposeQueue是干啥的 其中一个元素就这么复杂：')
        for(ob in data){
            console.log(ob)
        }
        var type = data.type

        console.log('在rejectDisposeQueue里面记录对象的类型个数')
        if (newInfo[type]) {
            newInfo[type]++
        } else {
            newInfo[type] = 1
    ...
    i = n
    if (diff) {
        console.log('diff为trru的时候 欢迎进入视图更新Do函数')
        while (data = disposeQueue[--i]) {
...
function disposeData(data) {
    ...
    console.log('这个disposeData他是一个删除逻辑  清除了data.element ')
